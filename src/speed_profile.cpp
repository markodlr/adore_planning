                    ...
                    const dynamics::ComfortSettings& comfort_settings, const SpeedProfileConfig& config )
{
  ...
  // 6. DP
  SpeedProfile dp_trajectory = solve_dp( config, comfort_settings, ego_s, ego_v, 0.0 /* ego_a? if avail */, t_samples, forbidden, limits );
...
void
apply_temporal_smoothing( SpeedProfile& new_profile, const SpeedProfile& previous_profile, const SpeedProfileConfig& config )
{
  if( previous_profile.empty() || new_profile.empty() )
  {
    return;
  }

  // Helper to sample previous profile
  auto get_prev_speed = [&]( double s_query ) -> std::optional<double> {
    if( previous_profile.empty() )
      return std::nullopt;
    if( s_query < previous_profile.front().s || s_query > previous_profile.back().s )
      return std::nullopt;
    auto it = std::lower_bound( previous_profile.begin(), previous_profile.end(), s_query,
                                []( const auto& p, double s ) { return p.s < s; } );
    if( it == previous_profile.begin() )
      return it->v;
    auto   prev = std::prev( it );
    double r    = ( s_query - prev->s ) / ( it->s - prev->s );
    return prev->v + r * ( it->v - prev->v );
  };

  for( auto& p : new_profile )
  {
    if( auto v_prev = get_prev_speed( p.s ); v_prev )
    {
      // Tunnel bounds around DP solution
      double v_min = std::max( 0.0, p.v - config.speed_tunnel_width );
      double v_max = p.v + config.speed_tunnel_width;

      // Prefer previous solution, clamped to tunnel
      p.v = std::clamp( *v_prev, v_min, v_max );
    }
  }
}
