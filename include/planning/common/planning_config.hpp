...
struct SpeedProfileConfig
{
  ...
  // DP weights
  double w_dp_progress            = 1000.0; // reward progress (negative cost)
  double w_dp_speed               = 1.0;    // penalize dv
  double w_dp_accel               = 1.0;    // penalize a^2
  double w_dp_jerk                = 1.0;    // penalize j^2
  double w_dp_limit_violation     = 1000.0; // penalize exceeding v_limit
  ...
  // QP weights
  double w_qp_track_dp        = 0.01;
  double w_qp_accel           = 1.0;
  double w_qp_jerk            = 10.0;
  double w_qp_v0              = 10.0;   // fix initial velocity
  double w_qp_a0              = 1.0;    // fix initial accel
  double w_qp_limit_violation = 1000.0; // soft constraint for speed limit
